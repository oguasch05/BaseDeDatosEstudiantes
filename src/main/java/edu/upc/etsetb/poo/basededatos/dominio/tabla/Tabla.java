...
public class Tabla {
    ...
    public List<FilaDatos> buscaTodo() {
        List<FilaDatos> filascopy = new LinkedList<>(filas);
        return filascopy;
    ...
    public List<FilaDatos> busca(Criterio criterio) {
        List<FilaDatos> lista = this.buscaTodo();
        if (criterio != null) {
            for (int i = 0; i < lista.size(); i++) {
                if (!criterio.esCumplido(lista.get(i))) {
                    lista.remove(i);
                    i--;
                }
            }
    ...
    public List<FilaDatos> elimina(Criterio criterio) {
        List<FilaDatos> lista = new LinkedList<>();
        if (criterio != null) {
            for (int i = 0; i < filas.size(); i++) {
                if (criterio.esCumplido(filas.get(i))) {
                    lista.add(filas.get(i));
                    filas.remove(i);
                    i--;
                }
            }

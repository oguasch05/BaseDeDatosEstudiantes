 ...
 * facilitará la comunicación entre la
 * {@link edu.upc.etsetb.poo.basededatos.iu.InterfazUsuario} y los demás
...
public class Controlador {
     ...
     *                 "duración" }</code>
     * @throws edu.upc.etsetb.poo.basededatos.dominio.BaseDatosException
     */
    public void crearTabla(String[] palabras) {
        ...
        String nombre;
        boolean unica;
        Integer length = palabras.length;
        if (length < 3) {
            iu.println("Error en formato del comando");
        } else if (tablas.keySet().contains(nombreTabla)) {
            iu.println("Error en formato del comando");
        } else {
            for (int i = 2; i < palabras.length; i++) {
                unica = false;
                if (palabras[i] != null) {
                    if (palabras[i].charAt(0) == SIMB_CLAVE_ÚNICA) {
                        nombre = palabras[i].substring(1);
                        unica = true;
                    } else {
                        nombre = palabras[i];
                    }
                    Clave clave = new Clave(nombre, unica);
                    try {
                        esquema.addClave(clave);
                    } catch (ClaveYaExisteException e) {
                        iu.println("Clave " + nombre + " ya existe");
                        palabras[i] = null;
                        this.crearTabla(palabras);
                    }
                }
            }
            Tabla tabla = new Tabla(nombreTabla, esquema);
    ...
    public void anyadir(String[] palabras) {
        ...
        if (palabras.length < 3) {
            ...
            for (int i = 2; i < palabras.length; i++) {
                String[] claveValor = palabras[i].split("=");
                fila.put(claveValor[0], claveValor[1]);
            }
            try {
                tablas.get(nombreTabla).anyade(fila);
            } catch (ValorClaveUnicaException e) {
                iu.println("ValorClaveUnicaException");
            } catch (ClaveInexistenteException e) {
                iu.println("ClaveInexistenteException");
            }
        }
    ...
    public List<String> getCabeceras(String nombreTabla) {
        if (tablas.containsKey(nombreTabla)) {
            List<String> cabeceras = tablas.get(nombreTabla).getCabeceras();
            return cabeceras;
        } else {
            return null;
        }
    ...
    public static Criterio interpretaCriterio(String[] palabras) {
        if (palabras.length < 3) {
            ...
            String c = null;
            if (palabras[2].contains(SIMB_IGUAL)) {
                c = SIMB_IGUAL;
                String[] nombreValor = palabras[2].split(c);
                return new CriterioIgual(nombreValor[0], nombreValor[1]);
            } else if (palabras[2].contains(SIMB_CONTIENE)) {
                c = SIMB_CONTIENE;
                String[] nombreValor = palabras[2].split(c);
                return new CriterioContiene(nombreValor[0], nombreValor[1]);
            } else if (palabras[2].contains(SIMB_MAYOR_QUE)) {
                c = SIMB_MAYOR_QUE;
                String[] nombreValor = palabras[2].split(c);
                return new CriterioMayorQue(nombreValor[0], nombreValor[1]);
            } else if (palabras[2].contains(SIMB_MENOR_QUE)) {
                c = SIMB_MENOR_QUE;
                String[] nombreValor = palabras[2].split(c);
                return new CriterioMenorQue(nombreValor[0], nombreValor[1]);
            } else {
                return null;
            }
     ...
     * criterio definido en el comando, invocando al método null null null null
     ...
     * null null null null null null null null null null null null null null
     * null null null null null null null null null null null null null null
     * null null null null null null null     {@link edu.upc.etsetb.poo.basededatos.iu.InterfazUsuario#presentaResultados(java.util.List, 
     * java.util.List) }.</p>
     *
     * <p>
     * Formato: <code>nombreTabla busca</code> ó
     * <code>nombretabla busca criterio=valorBuscado</code> (el valor de
     * criterio debe a ser igual a valorBuscado) ó
     * <code>nombreTabla busca criterio#valorBuscado</code> (el valor de
     * criterio debe contener a valorBuscado) o
     * <code>nombreTabla busca criterio>valorBuscado</code> (el valor del
     * criterio debe ser mayor que valorBuscado) o
     * <code>nombreTabla busca criterio<valorBuscado</code> (el valor del
     * criterio debe ser menor que valorBuscado.</p>
     *
     * <p>
     ...
     *
     * <p>
     * Las tablas se mostrarán invocando al método null null null null null null
     * null null null null null null null null null null null null null null
     * null null null null null null null null null null null null null null
     * null null null null null null null null null null null null null null
     * null null null null null null null null null null null null null null
     * null null null null null null null null null null null null null null
     * null null null null null null null null null null null null null null
     * null null null null null     {@link edu.upc.etsetb.poo.basededatos.iu.InterfazUsuario#presentaResultados(java.util.List, 
     * java.util.List) }</p>
     *
     * <p>
     * Ejemplo:</p>
     ...
     * @return lista de filas de datos que cumplen el criterio o lista vacía si
     * se intenta buscar en una tabla que NO existe
     */
    public List<FilaDatos> buscar(String[] palabras) {
        if (!tablas.containsKey(palabras[0])) {
            iu.println("Tabla Erronea");
            return new LinkedList<>();
        } else {
            List<FilaDatos> lista = tablas.get(palabras[0]).busca(interpretaCriterio(palabras));
            iu.presentaResultados(getCabeceras(palabras[0]), lista);
            return lista;
        }
    ...
    public List<FilaDatos> eliminar(String[] palabras) {
        ...
        if (palabras.length < 3) {
            iu.println("Error en formato del comando");
            return new LinkedList<>();
        } else if (!tablas.containsKey(nombreTabla)) {
            iu.println("No existe la tabla " + nombreTabla);
            return new LinkedList<>();
        } else {
            Criterio criterio = interpretaCriterio(palabras);
            Tabla tabla = tablas.get(nombreTabla);
            List<FilaDatos> lista = tabla.elimina(criterio);
            iu.println("Eliminadas " + lista.size() + " filas");
            return lista;
        }
    ...
    /**
     * <p>
     * Gestiona el comando 'exporta': guarda en un archivo de disco (cuyo nombre
     * se pasa como parámetro) la secuencia de comandos (uno por línea) que
     * generaría las tablas que hay en memoria, con su contenido actual.</p>
     * <p>
     * Por ejemplo, si tuviéramos las siguientes tablas en memoria:</p>
     * <pre>
     * TABLA coches:
     * =================================================
     ...
     * </pre>
     * <p>
     * Y se invocara el método <code>exporta("tablas.txt")</code>, se generaría
     * un archivo de texto llamado 'tablas.txt' cuyo contenido sería el
     * siguiente:</p>
     * <pre>
     ...
     * </pre>
     *
     * @param archivo Nombre del archivo en el que se guardará el archivo de
     * exportación
     */
    public void exporta(String archivo) {
        ...
        try {
            ...
            for (String tabla : tablas.keySet()) {
                comando.append(tabla).append(" ");
                comando.append(CMD_CREA_TABLA);
                Iterator<Clave> it = tablas.get(tabla).getEsquema().iterator();
                while (it.hasNext()) {
                    Clave next = it.next();
                    comando.append(" ");
                    if (next.isUnica()) {
                        comando.append(SIMB_CLAVE_ÚNICA);
                    }
                    comando.append(next.getNombre());
                }
                pStream.println(comando);
                comando.setLength(0);
                for (int i = 0; i < tablas.get(tabla).buscaTodo().size(); i++) {
                    comando.append(tabla).append(" ").append(CMD_AÑADE).append(" ");
                    comando.append(tablas.get(tabla).buscaTodo().get(i).clavesTexto());
                    pStream.println(comando);
                    comando.setLength(0);
                }
    ...
    /**
     * <p>
     * Ordena la tabla cuyo nombre coincide con <code>palabras[0]</code> según
     * la clave cuyo nombre coincide con <code>palabras[2]</code></p>
     *
     * <p>
     * Por defecto se ordena en orden ascendente, pero si se pasa la palabra
     * 'desc' dentro de <code>palabras[3]</code>, se ordenan en orden
     * descendente.</p>
     *
     * <p>
     * Si la tabla cuyo nombre se pasa en el array de Strings no existe, invocar
     * al método iu.println() con el argumento "Error. Tabla inexistente"</p>
     *
     * <p>
     ...
     * invocar al método iu.println() con el argumento "Error. Columna
     * inexistente "</p>
     *
     * @param palabras Array con las palabras pertenecientes a un comando, por
     * ejemplo:
     * <code>{ "peliculas", "ordena", "título" } o { "peliculas", "ordena", "año", "desc" }</code>
     */
    public void ordenar(String[] palabras) {
        String nombreTabla = palabras[0];
        String columna = palabras[2];
        if (!tablas.containsKey(nombreTabla)) {
            iu.println("Error. Tabla inexistente");
        } else if (!tablas.get(nombreTabla).getEsquema().contieneClave(columna)) {
            iu.println("Error. Columna inexistente");
        } else {
            boolean asc = !(palabras.length > 3 && palabras[3].equals("desc"));
            tablas.get(nombreTabla).ordena(columna, asc);
        }
